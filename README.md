...
- **End-of-session summary** — a structured recap (locations visited, GM notes, clues revealed/hidden, battle stats, XP awarded) computed from the session's event log, shown automatically when a session ends and reopenable anytime from the History Log's "Summary" button.
